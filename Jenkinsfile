pipeline {
    agent {
        docker {
            label "devel10"
            image "docker-i.dbc.dk/iscrum-build-tools:latest"
            alwaysPull true
            reuseNode true
            args '-u isworker --network host -e USER=isworker -v /var/run/docker.sock:/var/run/docker.sock:rw,z -v /home/isworker/.m2:/home/isworker/.m2:rw,z'
        }
    }
    tools {
        maven "maven 3.5"
    ...
    environment {
        ...
        DOCKER_BUILD_PREFIX = "borchk-java-local"
        ...
        stage("WAR build") {
            steps {
                ...
                script {
                    ...
                    currentBuild.displayName = "Building ${env.BUILD_TAG}"
                }
                ansiColor("xterm") {
                    sh """
                        mvn -T 1.5C -B clean && \
                        mvn -T 1.5C -B org.jacoco:jacoco-maven-plugin:prepare-agent package
                       """
                }
        ...
        stage("Docker Push") {
            ...
            steps {
                script {
                    ...
                    // to make the seperation between "borchk-local" and "docker-i.dbc.dk" very clear.
            ...
            steps {
                ...
                script {
                    ...
                    ansiColor("xterm") {
                        sh """
                            mvn -T 1.5C -B clean && \
                            mvn -T 1.5C -B org.jacoco:jacoco-maven-plugin:prepare-agent deploy 
                           """
                    }
                }
            }
...
def getVersion() {
    return (readMavenPom().version).replace("-SNAPSHOT", "")
}
